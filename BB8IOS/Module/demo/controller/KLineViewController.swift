...
extension KLineViewController {
    ...
    func fetchChartDatas() {
        ...
        let sq = symbol.components(separatedBy: "-")

        let range = self.getRange(timeType: self.times[self.selectedTime])
        
        NetworkTool.getKlineChartData(market: "okex", symbol:sq[0], quote: sq[1], range: range, size:limit){
    ...
    //获取range
    func getRange(timeType : String) -> Int{
...
//        self.view.backgroundColor = UIColor(hex: 0x232732)
        ...
        let item2=UIBarButtonItem(customView: self.buttonTime)
...
extension KLineViewController: SettingListViewDelegate {
...
extension KLineViewController: ChartStyleSettingViewDelegate {
