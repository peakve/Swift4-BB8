...
class WeexViewController: UIViewController,UIGestureRecognizerDelegate {
    ...
    func render(){
        ...
        //TODO
        let bundleUrl=String.init(format: "file://%@/bundlejs/", Bundle.main.bundlePath)
        options["bundleUrl"]=bundleUrl;

        instance!.render(with: url, options: options, data: nil)
        
//        options:@{@"merchantNo":self.merchantNo,@"mobileNo":self.mobileNo,@"legalName":self.legalName
